...
import { DISTRIBUTION_WALLETNUM, LIL_JIT_MODE, PRIVATE_KEY, RPC_ENDPOINT, RPC_WEBSOCKET_ENDPOINT, SWAP_AMOUNT, VANITY_MODE, SKIP_DISTRIBUTION, REUSE_WALLETS, WALLET_MIN_SOL, UNDERFUNDED_MODE, WALLET_LIMIT } from "./constants"
...
const main = async () => {
  ...
  const useExistingWallets = SKIP_DISTRIBUTION || REUSE_WALLETS
  const desiredWalletCount = WALLET_LIMIT > 0 ? WALLET_LIMIT : DISTRIBUTION_WALLETNUM
  const distributionCount = useExistingWallets ? 0 : desiredWalletCount
  const minimumSolAmount = (SWAP_AMOUNT + 0.01) * distributionCount + 0.04
  ...
  if (useExistingWallets) {
    console.log("Using existing wallets from keys/data.json")
    kps = loadExistingWallets(desiredWalletCount)
    if (!kps.length) {
    ...
    let result = await distributeSol(connection, mainKp, desiredWalletCount)
    if (!result) {
